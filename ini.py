...
logger = logging.getLogger('ansiblehost')
...
class InventoryParser(object):
    ...
    def load_from_file(self, filename="inventory", loader=None):
    ...
    def parse(self, lines, start_group='ungrouped'):
        '''
        ...
        for line in lines:
            ...
            line = line.strip()
            logger.debug("line {} group {}".format(line, groupname))
            ...
            if m:
                ...
                if state not in ['hosts', 'children', 'vars']:
                    title = ":".join(m.groups())
                    raise SyntaxError("Section [%s] has unknown type: %s" % (title, state))
                ...
                if groupname not in self.groups:
                    logger.debug("new group {}".format(groupname))
                ...
                raise SyntaxError("Invalid section entry: '%s'. Please make sure that there are no spaces" % line + \
            ...
            if state == 'hosts':
                ...
                logger.debug("new host {} group {}".format(hosts, groupname))
                for h in hosts:
                    self.groups[groupname].add_host(h)
                logger.debug("hosts in group {}: {}".format(groupname, self.groups[groupname].get_hosts()))
            ...
            # accepts a state that isn't handled above.
                ...
                raise SyntaxError("Entered unhandled state: %s" % (state))
        ...
        for g in pending_declarations:
            ...
            if decl['state'] == 'vars':
                raise SyntaxError("%s:%d: Section [%s:vars] not valid for undefined group: %s" % (self.filename, decl['line'], decl['name'], decl['name']))
            elif decl['state'] == 'children':
                raise SyntaxError("%s:%d: Section [%s:children] includes undefined group: %s" % (self.filename, decl['line'], decl['parent'], decl['name']))
    ...
    def _parse_group_name(self, line):
        ...
        raise SyntaxError("Expected group name, got: %s" % (line))
    ...
    def _parse_variable_definition(self, line):
        ...
        raise SyntaxError("Expected key=value, got: %s" % (line))
    ...
    def _parse_host_definition(self, line):
        ...
        try:
            ...
            logger.debug("tokesn {}".format(tokens))
        except ValueError as e:
            raise ValueError("Error parsing host definition '%s': %s" % (line, e))

        (hostnames, port) = self._expand_hostpattern(tokens[0])
        logger.debug("hostnames {}, port {}".format(hostnames, port))

        #get list of Host objecgts
        ...
        for t in tokens[1:]:
            if '=' not in t:
                raise ValueError("Expected key=value host variable assignment, got: %s" % (t))
            (k, v) = t.split('=', 1)
